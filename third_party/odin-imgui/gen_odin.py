...
g_processing_imgui_internal = False
...
# Store some info about what we were emitting to quickly find the offending entry in the json file
g_debug_parsing_file = ""
g_debug_emitting_kind = ""
g_debug_emitting_key = ""

def debug_set_emitting_key(emitting_key: str):
	global g_debug_emitting_key
	g_debug_emitting_key = emitting_key
def debug_set_emitting_kind(emitting_kind: str):
	global g_debug_emitting_kind
	g_debug_emitting_kind = emitting_kind
	debug_set_emitting_key("") # Clear key
def debug_set_parsing_file(parsing_file: str):
	global g_debug_parsing_file
	g_debug_parsing_file = parsing_file
	debug_set_emitting_kind("") # Clear kind

def die(message):
	print()
	print("FATAL ERROR: " + message)
	print()
	print(f"While parsing {g_debug_parsing_file}: {g_debug_emitting_kind} {g_debug_emitting_key}")
	print()
	nice_stack(1)
	exit(1)
...
def strip_prefix_optional(prefix: str, string: str) -> str | None:
...
def strip_suffix_optional(suffix: str, string: str) -> str | None:
...
def strip_circumfix_optional(prefix: str, suffix: str, string: str) -> str | None:
...
def make_type_odiney(type_str: str) -> str:
	# # nocheckin: Vector
	# if type_str.startswith("ImVector_"):
	# 	vector_type = type_str.removeprefix("ImVector_")
	# 	return f"Vector({make_type_odiney(vector_type)})"
...
def peek_named_type(type_desc) -> str | None:
...
def try_convert_enum_literal(name: str) -> str | None:
...
def get_array_count(type_desc) -> str | None:
...
# nocheckin: why would an ifdef not be allowed?
_allowed_ifdef = [
	...
	"IMGUI_DEBUG_HIGHLIGHT_ALL_ID_CONFLICTS", # nocheckin
...
def manually_set_define(define_name, value): processed_defines[define_name] = value
...
def condition_if(expression_str) -> bool:
	original_expression_str = expression_str
	# This is hardcoded for the path where we have [optional !, defined(, some_def, ), optional &&] and repeat
	try:
		while len(expression_str) > 0:
			# If we start with '!', we need to negate the expression later
			[expression_str, negate] = _chomp("!", expression_str)
			...
			assert ok
			# If the define str evaluates to false OR it evaluates to true and it is negated: return false (short circuit)
			if _ifdef(found_str) == negate: return False
			# If we next have &&, continue the loop
			[expression_str, ok] = _chomp("&&", expression_str)
			if ok: assert len(expression_str) > 0
	except:
		die(f"Failed to parse expression_str: '{original_expression_str}'")
		return False

def passes_conditionals(thing_with_conditionals) -> bool:
	"""If the dear_bindings object has a field called "conditionals", try to parse and evaluate it.
	Returns true if the condition passed, or if no conditional was on object."""
	...
	for conditional in thing_with_conditionals["conditionals"]:
		...
		condition_expression = conditional["expression"]

		if   condition_kind == "ifdef":  return     condition_ifdef(condition_expression)
		elif condition_kind == "ifndef": return not condition_ifdef(condition_expression)
		elif condition_kind == "if":     return     condition_if(condition_expression)
		elif condition_kind == "ifnot":  return not condition_if(condition_expression)
		else:
...
def ingest_and_write_defines(file: typing.IO, defines):
	debug_set_emitting_kind("define")

	write_section(file, "Defines")
	aligned = []

	for define in defines:
		debug_set_emitting_key(define["name"])
...
def write_enum_as_flags(file, enum, enum_field_prefix, name):
	...
	for element in enum["elements"]:
		...
		if bit_index != None:
			...
			if try_eval(element_value) != None:
				...
				append_aligned_field(aligned_flags, [f'{name}_{element_name}', f' :: i32({element_value}) // {extra_comment}'], element)
				continue
			else:
	...
	# Flags
	write_line_with_comments(file, f'{name} :: bit_set[{name.removesuffix("s")}; i32]', enum)
	write_line(file, f'{name.removesuffix("s")} :: enum i32 {{')
...
def write_enum_as_constants(file, enum, enum_field_prefix, name):
	write_line_with_comments(file, f'{name} :: distinct i32', enum)
	...
	for element in enum["elements"]:
		...
		is_enum_list = "|" in field_value # "ImBla | ImOtherThing | ImFoo"

		if not is_enum_list:
			append_aligned_field(aligned, [f'{name}_{field_name}', f' :: {name}({make_value_odiney(field_value)})'], element)
		else:
...
def write_enum(file: typing.IO, enum, enum_field_prefix: str, name: str):
	write_line_with_comments(file, f'{name} :: enum i32 {{', enum)
...
def write_enums(file: typing.IO, enums):
	debug_set_emitting_kind("enum")

	write_section(file, "Enums")
	for enum in enums:
		debug_set_emitting_key(enum["name"])
...
# These structs are defined in imgui_internal, and forward declared in imgui.
# If we emit bindings in both cases, we will have a conflict.
# So we skip them in imgui _if_ we are going to see them later in imgui_internal
...
_imgui_structs_skip = [
	"ImFontGlyph", # Contains a bit field. Manually created in `imgui_manual.odin`
]

def write_structs(file: typing.IO, structs):
	debug_set_emitting_kind("struct")

	write_section(file, "Structs")
	for struct in structs:
		debug_set_emitting_key(struct["name"])
		entire_name = struct["name"]

		if entire_name in _imgui_structs_skip: continue

		# # TODO[TS]: imvector
		# # We have a manually written parapoly vector binding (Vector(T))
		# if entire_name.startswith("ImVector_"): continue

		if not g_processing_imgui_internal and entire_name in _imgui_struct_skip_if_not_imgui_internal:
			continue
...
def write_functions(file: typing.IO, functions):
	debug_set_emitting_kind("function")
	...
	for function in functions:
		debug_set_emitting_key(function["name"])
...
def write_typedefs(file: typing.IO, typedefs):
	debug_set_emitting_kind("typedef")

	write_section(file, "Typedefs")
	aligned = []

	for typedef in typedefs:
		debug_set_emitting_key(typedef["name"])
...
def main():
	tested_against_version = "3.12.2"

	if not sys.version.startswith(tested_against_version):
		print("", file=sys.stderr)
		print(f"Warning: odin-imgui has been tested against Python version {tested_against_version}. (Yours: {sys.version})", file=sys.stderr)
	...
	parser.add_argument("--imgui_internal", required=True, help="The dear_bindings json file for imgui_internal.h")

	args = parser.parse_args()

	global g_processing_imgui_internal
	g_processing_imgui_internal = False

	manually_set_define("IMGUI_DISABLE_OBSOLETE_FUNCTIONS", True)

	# Read main imgui file
	debug_set_parsing_file("imgui.h")
	...
	# Read imgui_internal file
	g_processing_imgui_internal = True
	debug_set_parsing_file("imgui_internal.h")
